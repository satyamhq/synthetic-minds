<template>
  <div class="process-page">
    <!-- Top Navigation Bar -->
    <nav class="navbar">
      <div class="nav-brand" @click="goHome">SYNTHETIC MINDS</div>
      
      <!-- Step Indicator -->
      <div class="nav-center">
        <div class="step-badge">STEP 01</div>
        <div class="step-name">{{ $tm('main.stepNames')[0] }}</div>
      </div>
      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        <div class="panel-header">
          <div class="header-left">
            ...
            <span class="header-title">{{ $t('graph.panelTitle') }}</span>
          </div>
          <div class="header-right">
            <template v-if="graphData">
              <span class="stat-item">{{ graphData.node_count || graphData.nodes?.length || 0 }} {{ $t('step1.entityNodes') }}</span>
              <span class="stat-divider">|</span>
              <span class="stat-item">{{ graphData.edge_count || graphData.edges?.length || 0 }} {{ $t('step1.relationEdges') }}</span>
              <span class="stat-divider">|</span>
            </template>
            <div class="action-buttons">
                ...
                <button class="action-btn" @click="toggleFullScreen" :title="$t('graph.toggleMaximize')">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-hint">Graph construction starts automatically once completed</p>
          </div>
          
          <!-- Building without data yet -->
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  After uploading documents, LLM analyzes content to generate ontology schema (entity types + relation types).
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">API Description</div>
                <div class="detail-content">
                  Based on generated ontology, chunk documents and extract entities/relations into knowledge graph via OpenAI & Zep.
                </div>
              </div>
...
const initProject = async () => {
  ...
  if (paramProjectId === 'new') {
    // New project: fetch pending upload from store
...
const handleNewProject = async () => {
  ...
  if (!pending.isPending || pending.files.length === 0) {
    error.value = 'No files to upload. Please return to home page and select files.'
    loading.value = false
    return
  }
...
const fetchGraphData = async () => {
  try {
    // Fetch project info first to obtain graph_id
    const projectResponse = await getProject(currentProjectId.value)
...
const renderGraph = () => {
  ...
  // Create node map to lookup names
  const nodeMap = {}
  nodesData.forEach(n => {
    nodeMap[n.uuid] = n
  })
  ...
  // Draw edges (including clickable transparent line)
  ...
  // Transparent wide stroke for click hits
