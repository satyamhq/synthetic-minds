<template>
  <div class="env-setup-panel">
    <div class="scroll-container">
      ...
      <!-- Step 03: Generate Simulation Configuration -->
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <!-- Simulation Rounds Configuration - Displayed once configuration is generated -->
...
// Log deduplication: record last emitted info
...
watch(currentStage, (newStage) => {
  if (newStage === 'generating_profiles' || newStage === 'Generating Agent Profiles' || newStage.toLowerCase().includes('profile')) {
    phase.value = 1
  } else if (newStage === 'generating_config' || newStage === 'Generating Simulation Configuration' || newStage.toLowerCase().includes('config')) {
    phase.value = 2
  ...
  } else if (newStage === 'copying_scripts' || newStage === 'Preparing Simulation Scripts' || newStage.toLowerCase().includes('script')) {
    phase.value = 2 // Still in config phase
  }
})

// Calculate auto-generated rounds from config
const autoGeneratedRounds = computed(() => {
  ...
  // Ensure max rounds is at least 40 (recommended)
...
// Get username according to agent_id
...
// Calculate total related topics across profiles
...
const handleStartSimulation = () => {
  // Construct parameters passed to parent
  const params = {}
  ...
  if (useCustomRounds.value) {
    ...
    // User keeps auto rounds: omit max_rounds
...
const startPrepareSimulation = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Set expected total agents immediately
...
const pollPrepareStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Parse stage details and emit detailed log
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Emit profile progress log (only when count changes)
...
const fetchConfigRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Emit config phase log (deduplicated)
...
onMounted(() => {
  // Automatically start preparation workflow
  if (props.simulationId) {
