        ...
        <div v-if="reportOutline" class="report-content-wrapper">
          ...
          <div class="report-header-block">
            <div class="report-meta">
              <span class="report-tag">Simulation Report</span>
            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (Content generated, section may not be finalized) -->
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- Special banner when final answer -->
...
const toggleRawResult = (timestamp, event) => {
  ...
  // Wait for DOM update, adjust scroll position to maintain button location
...
const toolConfig = {
  ...
  'interview_agents': {
    ...
    color: 'green',
    icon: 'users' // Users icon - interviews
  },
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
    icon: 'zap' // Zap icon - quick search
  },
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    const queryMatch = text.match(/(?:Analysis Question|Query):\s*(.+?)(?:\n|$)/i)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    // Extract prediction scenario
    const reqMatch = text.match(/(?:Prediction Scenario|Scenario):\s*(.+?)(?:\n|$)/i)
    if (reqMatch) result.simulationRequirement = reqMatch[1].trim()
    
    // Extract statistics
    const factMatch = text.match(/(?:Relevant Predictive Facts|Predictive Facts|Facts):\s*(\d+)/i)
    const entityMatch = text.match(/(?:Entities Involved|Entities):\s*(\d+)/i)
    const relMatch = text.match(/(?:Relationship Chains|Relations):\s*(\d+)/i)
    if (factMatch) result.stats.facts = parseInt(factMatch[1])
    if (entityMatch) result.stats.entities = parseInt(entityMatch[1])
    if (relMatch) result.stats.relationships = parseInt(relMatch[1])
    
    // Extract sub-questions
    const subQSection = text.match(/### (?:Analyzed Sub-questions|Sub-questions)\n([\s\S]*?)(?=\n###|$)/i)
    if (subQSection) {
    ...
    if (entitySection) {
      ...
      result.entities = entityBlocks.map(block => {
        ...
        const relatedMatch = block.match(/(?:Related Facts|Facts):\s*(\d+)/i)
        return {
    ...
    const relSection = text.match(/### (?:Relationship Chains|\[Relationship Chains\])\n([\s\S]*?)(?=\n###|$)/i)
    if (relSection) {
...
const parsePanorama = (text) => {
  ...
  try {
    // Extract query
    const queryMatch = text.match(/(?:Query|Search):\s*(.+?)(?:\n|$)/i)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    // Extract statistics
    const nodesMatch = text.match(/(?:Total Nodes):\s*(\d+)/i)
    const edgesMatch = text.match(/(?:Total Edges):\s*(\d+)/i)
    const activeMatch = text.match(/(?:Active Facts):\s*(\d+)/i)
    const histMatch = text.match(/(?:Historical\/Expired Facts|Historical Facts):\s*(\d+)/i)
    ...
    // Extract active facts
    const activeSection = text.match(/### (?:Active Facts|\[Active Facts\])[\s\S]*?\n([\s\S]*?)(?=\n###|$)/i)
    if (activeSection) {
    ...
    const histSection = text.match(/### (?:Historical\/Expired Facts|Historical Facts|\[Historical Facts\])[\s\S]*?\n([\s\S]*?)(?=\n###|$)/i)
    if (histSection) {
    ...
    const entitySection = text.match(/### (?:Involved Entities|\[Involved Entities\])\n([\s\S]*?)(?=\n###|$)/i)
    if (entitySection) {
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // Extract interviewee counts
    const countMatch = text.match(/\*\*(?:Interviewees|Number of Interviewees):\*\*\s*(\d+)\s*\/\s*(\d+)/i)
    if (countMatch) {
    ...
    // Extract interview selection rationale
    const reasonMatch = text.match(/### (?:Interviewees Selection Rationale|Selection Rationale)\n([\s\S]*?)(?=\n---\n|\n### (?:Interview Transcript|Transcript))/i)
    if (reasonMatch) {
      result.selectionReason = reasonMatch[1].trim()
    }
    
    // Parse selection rationale per interviewee
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Example: 1. **Name (index=1)**: Rationale...
        ...
        // Example: - Select Name (index 0): Rationale...
        if (!headerMatch) {
        ...
        // Example: - **Name (index 0)**: Rationale...
        if (!headerMatch) {
        ...
        if (name) {
          // Save previous person rationale
        ...
        } else if (currentName && line.trim() && !line.match(/^(?:Not selected|In summary|Final choice|Conclusion)/i)) {
          // Rationale continuation lines
          currentReason.push(line.trim())
        }
      }
      
      // Save last person rationale
      if (currentName && currentReason.length > 0) {
        reasons[currentName] = currentReason.join(' ').trim()
      }
    ...
    const interviewBlocks = text.split(/#### (?:Interview|Interview Record)\s*#?\d+[:：]/i).slice(1)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (nameRoleMatch) {
        ...
        // Set person selection rationale
        interview.selectionReason = individualReasons[interview.name] || ''
      }
      
      // Extract bio
      const bioMatch = block.match(/_(?:Bio|Introduction):\s*([\s\S]*?)_\n/i)
      if (bioMatch) {
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // If first question preceded by "1.", handle specially
      ...
      const answerMatch = block.match(/\*\*A:\*\*\s*([\s\S]*?)(?=\*\*(?:Key Quotes|Quotes)|$)/i)
      if (answerMatch) {
        const answerText = answerMatch[1].trim()
        
        // Split Twitter and Reddit answers
        const twitterMatch = answerText.match(/(?:\[Twitter Answer\]|【Twitter】)\n?([\s\S]*?)(?=(?:\[Reddit Answer\]|【Reddit】)|$)/i)
        const redditMatch = answerText.match(/(?:\[Reddit Answer\]|【Reddit】)\n?([\s\S]*?)$/i)
        ...
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer
          if (interview.redditAnswer && !interview.redditAnswer.includes('No response')) {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
          if (interview.twitterAnswer && !interview.twitterAnswer.includes('No response')) {
            interview.redditAnswer = interview.twitterAnswer
          }
        } else if (!twitterMatch && !redditMatch) {
          // Fallback: entire text as answer
          interview.twitterAnswer = answerText
        }
      }
      
      // Extract key quotes
      const quotesMatch = block.match(/\*\*(?:Key Quotes|Quotes):\*\*\n([\s\S]*?)(?=\n---|\n####|$)/i)
      if (quotesMatch) {
    ...
    const summaryMatch = text.match(/### (?:Interview Summary & Core Views|Interview Summary)\n([\s\S]*?)$/i)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    const countMatch = text.match(/(?:Found\s*(\d+)\s*(?:results|items)|(\d+)\s*results)/i)
    if (countMatch) result.count = parseInt(countMatch[1])
    
    // Extract related facts
    const factsSection = text.match(/### (?:Related Facts|Facts):\n([\s\S]*)$/i)
    if (factsSection) {
      const lines = factsSection[1].split('\n').filter(l => l.match(/^\d+\./))
      result.facts = lines.map(l => l.replace(/^\d+\.\s*/, '').trim()).filter(Boolean)
    }
    
    // Extract edge info if available
    const edgesSection = text.match(/### (?:Related Edges|Edges):\n([\s\S]*?)(?=\n###|$)/i)
    if (edgesSection) {
    ...
    const nodesSection = text.match(/### (?:Related Nodes|Nodes):\n([\s\S]*?)(?=\n###|$)/i)
    if (nodesSection) {
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t.includes('No response') || t.includes('no response') || t === '[No reply]'
    }
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // If no numbering or single match, return whole text
      if (matches.length <= 1) {
        const cleaned = answerText
          .replace(/^(?:Question|Q)\d+[:：]\s*/i, '')
          .replace(/^\d+\.\s+/, '')
          .trim()
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Split failed: return full answer for first, empty for rest
      return qIdx === 0 ? answer : ''
    }
    
    // Check if question has dual-platform answers
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Both platforms have substantive responses
...
// Currently active step (displayed on top)
...
const renderMarkdown = (content) => {
  ...
  // Remove top level h2 since section title is rendered externally
  ...
  // Clean <p><br> preceding block elements
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Clean consecutive <br> tags
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Clean <br> before paragraph after block elements
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix discontinuous ordered list numbering
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // If task finished, scroll to top; otherwise scroll to bottom
            if (isComplete.value) {
...
const extractFinalContent = (response) => {
  ...
  // Find content following Final Answer:
  ...
  // Find content following final answer
  const chineseFinalMatch = null
  if (chineseFinalMatch) {
    return chineseFinalMatch[1].trim()
  }
  
  // If starting with ##, # or >, likely raw markdown content
  const trimmedResponse = response.trim()
  if (trimmedResponse.match(/^[#>]/)) {
    return trimmedResponse
  }
  
  // If long content with markdown, strip thought trace
...
.section-number {
  ...
  font-size: 16px;
  color: #9CA3AF; /* Dark gray, constant across states */
  font-weight: 500;
}
