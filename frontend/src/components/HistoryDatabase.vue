<template>
  ...
  >
    <!-- Background Decoration: Technical Grid Lines (only when items exist) -->
    ...
    <!-- Card Container (only when items exist) -->
...
const containerStyle = computed(() => {
  ...
  // Expanded state: compute dynamic height by card count
...
// Navigate to Step 1 Graph Build
const goToProject = () => {
...
// Navigate to Step 2 Environment Setup
...
const loadHistory = async () => {
  try {
    ...
    console.error('Failed to load historical simulations:', error)
    projects.value = []
  } finally {
    loading.value = false
  }
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // If animating, queue pending state
        if (isAnimating) return
        
        // No-op if target state matches current
        if (shouldExpand === isExpanded.value) {
          pendingState = null
          return
        }
        
        // Debounce state transition to prevent flickering
        // Short delay on expand (50ms), longer on collapse (200ms)
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Unlock after animation and check pending states
          setTimeout(() => {
            isAnimating = false
            
            // Check if new pending state after animation
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Short delay before executing to prevent fast flips
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
...
// Reload data when returning to home view
...
// Reload data on activated if keep-alive
onActivated(() => {
  loadHistory()
})
...
/* Simplified display when no items */
...
.grid-pattern {
  ...
  /* Position from top-left, height expands at bottom */
...
.cards-container {
  ...
  /* min-height dynamically computed via JS */
}
...
/* Low saturation color scheme - Morandi tones */
