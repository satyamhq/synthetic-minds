...
// Dismiss simulation finished banner
...
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Transition from simulating to idle: show finished banner
...
const renderGraph = () => {
  ...
  // Process edge data, count edges and index between pairs
  const edgePairCount = {}
  const selfLoopEdges = {} // Self-loop edges grouped by node
  const tempEdges = edgesData
    .filter(e => nodeIds.has(e.source_node_uuid) && nodeIds.has(e.target_node_uuid))
  
  // Count edges between pairs, collect self-loops
  tempEdges.forEach(e => {
  ...
  // Record current edge index between pairs
  const edgePairIndex = {}
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      // Self-loop edge - add single aggregated self-loop per node
      ...
      edges.push({
        ...
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // Store detailed info of all self-loops
        }
      })
      return
    }
    ...
    // Determine if edge direction matches normalized direction (sourceUUID < targetUUID)
    const isReversed = e.source_node_uuid > e.target_node_uuid
    
    // Calculate curvature: fan out for multiple edges, straight for single
    let curvature = 0
    if (totalCount > 1) {
      // Distribute curvature evenly for clear distinction
      // Curvature range expands as edge count increases
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      // Flip curvature if edge direction opposes normalized direction
      // Ensures all edges distribute in unified frame without overlapping
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // Dynamically adjust distance according to edge count
    ...
    // Center gravity to cluster independent groups towards center
  ...
  const getLinkPath = (d) => {
    ...
    // Detect self-loops
    if (d.isSelfLoop) {
      // Self-loop: draw arc originating from and returning to node
      const loopRadius = 30
      // Depart from node right side, circle back
      const x1 = sx + 8  // Start offset
      const y1 = sy - 4
      const x2 = sx + 8  // End offset
      const y2 = sy + 4
      // Draw arc for self loop (sweep-flag=1 clockwise)
    ...
    // Calculate curve control points dynamically
  ...
  // Keep reference for external visibility control
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        // Check if actual drag initiated (moved past threshold)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // Restart simulation only upon actual drag
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Decay simulation only if actually dragged
        if (d._isDragging) {
  ...
  simulation.on('tick', () => {
    ...
    // Update edge label positions (horizontal for readability)
...
/* Simulation finished banner styles */
