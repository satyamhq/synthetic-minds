...
def state_with_snapshots(snapshots=None):
    return {
        "schema_version": 1,
        "repository": "satyamhq/synthetic-minds",
        "timezone": "UTC",
...
class StarHistoryBehaviorTests(unittest.TestCase):
    ...
    def test_svg_is_accessible_self_contained_and_deterministic(self):
        ...
        self.assertIn(b"satyamhq/synthetic-minds", light)
        ...
        legend_labels = [
            ...
            for element in root.iter()
            if element.tag.rsplit("}", 1)[-1] == "text"
            and element.text == "satyamhq/synthetic-minds"
        ]
        self.assertEqual(legend_labels, ["satyamhq/synthetic-minds"])
        for element in root.iter():
