"""
Report API routes
Provides simulation report generation, retrieval, and conversational Q&A endpoints
"""
...
logger = get_logger('synthetic_minds.api.report')
...
def generate_report():
    ...
    This is a long-running task; returns task_id immediately
    ...
    try:
        ...
        logger.error(f"Failed to launch report generation task: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
...
def download_report(report_id: str):
    ...
    Returns Markdown file attachment
    """
    try:
        report = ReportManager.get_report(report_id)
        ...
        if not os.path.exists(md_path):
            # If Markdown file missing, generate temporary file
            import tempfile
...
def chat_with_report_agent():
    ...
    Report Agent can autonomously invoke retrieval tools to answer queries
    
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",        // Required, simulation ID
            "message": "Please explain the sentiment trajectory",    // Required, user message
    ...
    try:
        ...
        # Retrieve simulation and project context
        manager = SimulationManager()
        state = manager.get_simulation(simulation_id)
        ...
        # Initialize Agent and execute chat turn
        agent = ReportAgent(
            graph_id=graph_id,
            simulation_id=simulation_id,
            simulation_requirement=simulation_requirement
...
def get_report_progress(report_id: str):
    """
    Get realtime report generation progress
...
def get_report_sections(report_id: str):
    """
    Get list of generated sections (streamed output)
    
    Frontend polls this endpoint for incremental section delivery
...
# ============== Report Status Inspection Endpoints ==============

@report_bp.route('/check/<simulation_id>', methods=['GET'])
def check_report_status(simulation_id: str):
    """
    Check whether simulation has report and report status
    
    Used by frontend to check if Interview feature is unlocked
...
# ============== Agent Structured Log Endpoints ==============
...
def get_agent_log(report_id: str):
    ...
    Realtime event stream of actions including:
    - Report start, outline planning start/done
    - Section start, tool calls, LLM responses, completion
    - Report completed or failed
    
    Query Parameters:
        from_line: Starting line offset (optional, default 0)
...
def get_console_log(report_id: str):
    ...
    Realtime console log lines (INFO, WARNING, etc.),
    distinct from structured agent-log JSON format.
    Plain text console output format.
    ...
    Returns:
        {
            ...
            "data": {
                "logs": [
                    "[19:46:14] INFO: Search complete: 15 facts found",
...
def stream_console_log(report_id: str):
    """
    Get complete console log stream
