...
Runs simulations in background processes and logs agent actions with realtime status monitoring.
"""
...
logger = get_logger('synthetic_minds.simulation_runner')

# Flag indicating whether cleanup handler is registered
...
class SimulationRunState:
    ...
    # Per-platform round and time tracking for parallel display
    twitter_current_round: int = 0
    reddit_current_round: int = 0
    twitter_simulated_hours: int = 0
    reddit_simulated_hours: int = 0
    
    # Platform execution flags
    twitter_running: bool = False
    reddit_running: bool = False
    twitter_actions_count: int = 0
    reddit_actions_count: int = 0
    
    # Platform completion flags from simulation_end events
    ...
    # Process ID for lifecycle control
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            # Independent rounds and elapsed simulation time per platform
    ...
    def to_detail_dict(self) -> Dict[str, Any]:
        """Detailed state dictionary including recent actions."""
        result = self.to_dict()
...
class SimulationRunner:
    ...
    Responsibilities:
    1. Execute OASIS simulation in background process
    2. Parse execution logs and record agent actions
    3. Expose real-time query interface
    4. Manage lifecycle (start/stop/pause/resume)
    """
    ...
    def get_run_state(cls, simulation_id: str) -> Optional[SimulationRunState]:
        """Get run state for simulation."""
        if simulation_id in cls._run_states:
            return cls._run_states[simulation_id]
        
        # Attempt load from persisted file
    ...
    def start_simulation(
        ...
        max_rounds: int = None,  # Max rounds (optional cap for long runs)
        ...
        Args:
            ...
            enable_graph_memory_update: Whether to dynamically stream activities to Zep graph
            graph_id: Zep graph ID (required when enabled)
        ...
        # Cap rounds if max_rounds specified
        ...
        with cls._finalization_lock(simulation_id):
            ...
            if (
                ...
                raise ValueError(f"Simulation is already running or finalizing: {simulation_id}")
        ...
        # Determine execution script from backend/scripts/
        if platform == "twitter":
        ...
        if not os.path.exists(script_path):
            ...
            if cleanup_error is not None:
                state.error += f"; Zep graph ingestion cleanup failed: {cleanup_error}"
            with cls._finalization_lock(simulation_id):
                cls._save_run_state(state)
                cls._sync_simulation_status(
                    simulation_id,
        ...
        try:
            # Build command line using absolute paths
            # Directory structure:
            #   twitter/actions.jsonl - Twitter action log
            #   reddit/actions.jsonl  - Reddit action log
            #   simulation.log        - Main runner log
            
            cmd = [
                sys.executable,  # Python interpreter
                script_path,
                "--config", config_path,  # Absolute configuration file path
            ]
            ...
            # Direct stdout/stderr to file to prevent pipe buffer deadlocks
            main_log_path = os.path.join(sim_dir, "simulation.log")
            main_log_file = open(main_log_path, 'w', encoding='utf-8')
            
            # Set UTF-8 environment variables for subprocesses
            # Ensures cross-platform UTF-8 file reading in external dependencies
            env = os.environ.copy()
            env['PYTHONUTF8'] = '1'  # Python UTF-8 mode
            env['PYTHONIOENCODING'] = 'utf-8'  # Force UTF-8 IO encoding
            
            # Set working directory to simulation folder
            # Start new session group for complete process tree termination
            ...
            if process is not None and process.poll() is None:
                try:
                    ...
                    cleanup_errors.append(f"Subprocess termination failed: {error}")
            ...
            if main_log_file is not None:
                try:
                    ...
                    cleanup_errors.append(f"Log close failed: {error}")
            if cls._graph_memory_enabled.get(simulation_id, False):
                try:
                    ZepGraphMemoryManager.stop_updater(simulation_id)
                    cls._graph_memory_enabled.pop(simulation_id, None)
                except Exception as error:
                    cleanup_errors.append(f"Zep graph cleanup failed: {error}")
            state.runner_status = RunnerStatus.FAILED
            state.twitter_running = False
            state.reddit_running = False
            state.error = str(e)
    ...
    def _monitor_simulation(cls, simulation_id: str, locale: str = 'zh'):
        """Monitor simulation subprocess and stream action logs."""
        set_locale(locale)
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        ...
        try:
            while process.poll() is None:  # Process running
            ...
            # Final drain of log tail after process exit
            if os.path.exists(twitter_actions_log):
                ...
                if state.runner_status not in {
                    ...
                    if cls._graph_memory_enabled.get(simulation_id, False):
                        ...
                        try:
                            ...
                            cls._graph_memory_enabled.pop(simulation_id, None)
                            logger.info(
                                "Stopped graph memory update: simulation_id=%s",
                                simulation_id,
                            )
                        except Exception as error:
                            logger.error(f"Failed to stop graph memory updater: {error}")
                            desired_status = RunnerStatus.FAILED
                            error_message = f"Zep graph ingestion incomplete: {error}"
    ...
    def _read_action_log(
        ...
        """
        Read actions from log file incrementally.
        
        Args:
            log_path: Path to actions.jsonl
            position: File offset of previous read
            state: Simulation run state object
            platform: Platform name (twitter/reddit)
            
        Returns:
            New file offset position
        """
        # Check if graph memory updater is active
        ...
        try:
                ...
                for line in f:
                    ...
                    if line:
                        try:
                            ...
                            # Handle simulation event entries
                            if "event_type" in action_data:
                                event_type = action_data.get("event_type")
                                
                                # Handle simulation_end event
                                if event_type == "simulation_end":
                                    ...
                                    # Check if all enabled platforms completed execution
                                    ...
                                    if all_completed:
                                        ...
                                        logger.info(
                                            f"All platforms ended; waiting for process exit and graph ingestion: "
                                            f"{state.simulation_id}"
                                        )
                                
                                # Update round metrics from round_end events
                                elif event_type == "round_end":
                                    round_num = action_data.get("round", 0)
                                    simulated_hours = action_data.get("simulated_hours", 0)
                                    
                                    # Update platform-specific round and duration
                                    if platform == "twitter":
                                    ...
                                    # Aggregate round is max of platforms
                                    if round_num > state.current_round:
                                        state.current_round = round_num
                                    # Aggregate simulated hours is max of platforms
                            ...
                            # Stream action to Zep graph memory updater
                            if graph_updater:
            ...
            logger.warning(f"Failed reading action log: {log_path}, error={e}")
            return position
    
    @classmethod
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        """
        Check if all enabled platforms have finished simulation
        
        Check actions.jsonl existence to determine if platform is enabled
        ...
        # Detect active platforms by action log presence
        twitter_enabled = os.path.exists(twitter_log)
        reddit_enabled = os.path.exists(reddit_log)
        
        # If active platform is incomplete, return False
        if twitter_enabled and not state.twitter_completed:
            return False
        if reddit_enabled and not state.reddit_completed:
            return False
        
        # At least one platform active and completed
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        ...
        """
        if IS_WINDOWS:
            # Windows: Taskkill process tree (/F /T)
            ...
            # Unix: Process group termination (start_new_session)
    ...
    def stop_simulation(cls, simulation_id: str) -> SimulationRunState:
        ...
        if (
            ...
            if monitor.is_alive():
                ...
                raise SimulationStopPending(
                    f"Simulation is still stopping, graph ingestion did not complete within {wait_timeout:.0f}s"
                )
        else:
            ...
            with cls._finalization_lock(simulation_id):
                ...
                if cls._graph_memory_enabled.get(simulation_id, False):
                    try:
                        ...
                        state.reddit_running = False
                        state.completed_at = datetime.now().isoformat()
                        state.error = f"Zep graph ingestion incomplete: {error}"
                        cls._save_run_state(state)
                        cls._sync_simulation_status(
                            simulation_id,
                            RunnerStatus.FAILED,
        ...
        if state.runner_status == RunnerStatus.FAILED:
            raise RuntimeError(state.error or "Simulation failed to stop")
        if state.runner_status != RunnerStatus.STOPPED:
            raise RuntimeError(
                f"Simulation did not reach terminal state: {simulation_id}, status={state.runner_status}"
            )
    ...
    def _read_actions_from_file(
        ...
        """
        Read actions from an individual actions log file.
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Skip lifecycle event entries (simulation_start, round_start, round_end)
    ...
    def get_all_actions(
        ...
        """
        Retrieve complete action history across platforms.
        ...
        Returns:
            Action list ordered newest first
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        actions = []
        ...
        if not platform or platform == "twitter":
            actions.extend(cls._read_actions_from_file(
                twitter_actions_log,
                default_platform="twitter",  # Default platform
        ...
        if not platform or platform == "reddit":
            actions.extend(cls._read_actions_from_file(
                reddit_actions_log,
                default_platform="reddit",  # Default platform
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            ))
        
        # Fall back to legacy single action log if platform logs not present
        if not actions:
            actions_log = os.path.join(sim_dir, "actions.jsonl")
            actions = cls._read_actions_from_file(
                actions_log,
                default_platform=None,  # Legacy format with platform field
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            )
    ...
    def get_agent_stats(cls, simulation_id: str) -> List[Dict[str, Any]]:
        ...
        Returns:
            List of agent stat dictionaries
        """
        actions = cls.get_actions(simulation_id, limit=10000)
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Clean up simulation run logs (for clean restarts)
        ...
        Note: does not delete simulation_config.json or profile files
        ...
        logger.info(f"Cleaned simulation logs: {simulation_id}, removed: {cleaned_files}")
    ...
    def cleanup_all_simulations(cls):
        ...
        logger.info("Safely terminating simulation processes and draining graph memory...")
        ...
        for simulation_id in simulation_ids:
            try:
                ...
                cleanup_failed = True
                logger.error(
                    "Failed to clean up simulation, retaining state for retry: simulation_id=%s, error=%s",
                    simulation_id,
                    error,
                )
        ...
        if cleanup_failed:
            ...
            logger.error("Some simulations were not cleanly finalized")
        else:
            logger.info("Simulation processes and graph ingestion cleaned up successfully")
    ...
    def register_cleanup(cls):
        ...
        Called on Flask app startup to clean up simulation processes on shutdown
        """
        ...
        # In Flask debug mode, register only in reloader child process
        is_reloader_process = os.environ.get('WERKZEUG_RUN_MAIN') == 'true'
        is_debug_mode = os.environ.get('FLASK_DEBUG') == '1' or os.environ.get('WERKZEUG_RUN_MAIN') is not None
        
        # Register cleanup handler
        if is_debug_mode and not is_reloader_process:
            _cleanup_registered = True  # Prevent duplicate registration
            return
        ...
        def cleanup_handler(signum=None, frame=None):
            ...
            # Log only if processes exist to clean
            if cls._processes or cls._graph_memory_enabled:
                logger.info(f"Received signal {signum}, initiating cleanup...")
            cls.cleanup_all_simulations()
            
            # Call original handler for graceful shutdown
            if signum == signal.SIGINT and callable(original_sigint):
                ...
                # If original handler is not callable, exit directly
        ...
        try:
            ...
            # SIGHUP: Hangup signal (Unix)
            if has_sighup:
                signal.signal(signal.SIGHUP, cleanup_handler)
        except ValueError:
            # Not in main thread: rely on atexit
            logger.warning("Cannot register signal handlers outside main thread; relying on atexit")
    ...
    def get_env_status_detail(cls, simulation_id: str) -> Dict[str, Any]:
        ...
        Returns:
            Status dictionary containing status, twitter_available, reddit_available, timestamp
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Target platform (optional)
                ...
                - None: Interview both platforms and aggregate results
            timeout: Timeout in seconds
    ...
    def interview_agents_batch(
        ...
        Args:
            simulation_id: Simulation ID
            interviews: List of interview specs (agent_id, prompt, optional platform)
            platform: Default platform override
                - "twitter": Default to Twitter platform only
                - "reddit": Default to Reddit platform only
                - None: Interview both platforms for each agent
            timeout: Timeout in seconds
        ...
        logger.info(f"Sending batch Interview commands: simulation_id={simulation_id}, count={len(interviews)}, platform={platform}")
    ...
    def interview_all_agents(
        ...
        """
        Interview all agents globally using the same prompt

        Interview all agents in simulation using the same prompt

        Args:
            simulation_id: Simulation ID
            prompt: Interview question prompt (same question for all agents)
            platform: Target platform (optional)
                - "twitter": Only interview Twitter platform
                - "reddit": Only interview Reddit platform
                - None: Interview both platforms for each agent
        ...
        # Read agent configs from configuration file
        config_path = os.path.join(sim_dir, "simulation_config.json")
        if not os.path.exists(config_path):
    ...
    def close_simulation_env(
        ...
        """
        Close simulation environment (rather than terminating process)
        
        Send close command to simulation for graceful exit from waiting mode
        ...
        try:
            ...
            # Timeout might indicate the process is exiting
            return {
                "success": True,
    ...
    def get_interview_history(
        ...
        """
        Retrieve interview history records from databases.
        
        Args:
            simulation_id: Simulation ID
            platform: Platform filter (reddit/twitter/None)
            agent_id: Optional agent ID filter
            limit: Maximum count per platform
        ...
        if platform in ("reddit", "twitter"):
            ...
            # If platform not specified, query both
        ...
        # Cap total limit across platforms
