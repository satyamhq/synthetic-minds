...
Manages dual-platform simulations across Twitter and Reddit.
Executes simulation runners with intelligent parameter generation via LLM.
"""
...
logger = get_logger('synthetic_minds.simulation')
...
class SimulationStatus(str, Enum):
    ...
    COMPLETED = "completed"  # Naturally finished
    FAILED = "failed"
...
class SimulationState:
    ...
    def get_default_platform(self) -> str:
        """Return default platform based on enabled flags."""
...
class SimulationManager:
    ...
    3. Generate simulation parameters using LLM
    ...
    def prepare_simulation(
        ...
        """
        Automated preparation of simulation environment.
        
        Steps:
        1. Read and filter entities from Zep knowledge graph
        2. Generate OASIS Agent Profiles
        3. Generate simulation parameters using LLM
        4. Persist configs and profiles
        """
        ...
        try:
            ...
            if filtered.filtered_count == 0:
                ...
                state.error = "No matching entities found. Please check that the graph was built properly."
                self._save_simulation_state(state)
            ...
            # ========== Stage 3: Generate simulation parameters via LLM ==========
            if progress_callback:
                progress_callback(
                    "generating_config", 0,
            ...
            sim_params = config_generator.generate_parameters(
                simulation_id=simulation_id,
            ...
            with open(config_path, 'w', encoding='utf-8') as f:
                json.dump(sim_params.to_dict(), f, ensure_ascii=False, indent=2)
    ...
    def get_profiles(self, simulation_id: str, platform: str = None) -> List[Dict[str, Any]]:
        """Get agent profiles for a simulation platform."""
        state = self._load_simulation_state(simulation_id)
        if not state:
            raise ValueError(f"Simulation does not exist: {simulation_id}")
    ...
    def get_run_instructions(self, simulation_id: str) -> Dict[str, Any]:
        """Get command line execution instructions."""
        sim_dir = self._get_simulation_dir(simulation_id)
        config_path = os.path.join(sim_dir, "simulation_config.json")
        ...
        return {
            ...
            "instructions": (
                f"1. Activate environment: conda activate synthetic-minds\n"
