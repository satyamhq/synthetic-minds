...
Reads parameters from configuration file and automates the full simulation workflow.

Features:
- Keeps environment open for interactive command mode after simulation completes.
...
# Load .env file (containing OPENAI_API_KEY, etc.)
...
class UnicodeFormatter(logging.Formatter):
    """Custom formatter to render Unicode escape sequences as readable characters."""
...
class MaxTokensWarningFilter(logging.Filter):
    """Filter out camel-ai max_tokens warnings (tokens left unconstrained for model decision)."""
...
def setup_oasis_logging(log_dir: str):
    """Configure OASIS logging with deterministic file names."""
...
class IPCHandler:
    ...
    def _get_interview_result(self, agent_id: int) -> Dict[str, Any]:
        """Fetch latest interview result from SQLite database."""
...
class TwitterSimulationRunner:
    ...
    def _create_model(self):
        """Create OpenAI LLM model to drive Twitter agents."""
        openai_api_key = os.environ.get("OPENAI_API_KEY") or os.environ.get("LLM_API_KEY", "")
        openai_model = os.environ.get("OPENAI_MODEL") or os.environ.get("LLM_MODEL_NAME") or self.config.get("llm_model", "gpt-4o-mini")
    ...
    async def run(self, max_rounds: int = None):
        ...
        if self.wait_for_commands:
            print("\n" + "=" * 60)
            print("Entering wait-for-command mode - environment running")
