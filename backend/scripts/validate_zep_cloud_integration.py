...
# Capture the caller-supplied key before importing Synthetic Minds modules. app.config
...
BASELINE_EPISODES = [
    SourceEpisode(
        ...
        "Lanzhou Technologies (enterprise ID LZ-TECH) is a wind power intelligent maintenance company. "
        "The company is headquartered in Starport, and Lan Zhou has served as Chief Executive Officer (CEO) since 2024.",
        "baseline",
    ),
    SourceEpisode(
        "2026-01-12T10:00:00Z",
        "Lanzhou Technologies developed the SmartPatrol Platform (product ID ZHIXUN-01). "
        "SmartPatrol is currently in pilot stage, identifying turbine blade anomalies.",
        "baseline",
    ),
    SourceEpisode(
        "2026-01-20T14:00:00Z",
        "Lanzhou Technologies signed the HY-2026-01 agreement with Haiyue Energy (enterprise ID HY-ENERGY). "
        "Both parties will jointly deploy SmartPatrol at East Bay Wind Farm through December 31, 2026.",
        "baseline",
    ),
    SourceEpisode(
        "2026-02-02T09:30:00Z",
        "Yu Chen serves as project lead for Lanzhou Technologies SmartPatrol deployment at East Bay Wind Farm. "
        "Company executive operations remain under CEO Lan Zhou.",
        "baseline",
    ),
    SourceEpisode(
        "2026-02-20T18:00:00Z",
        "The SmartPatrol pilot at East Bay Wind Farm identified 12 blade anomalies, "
        "reducing Haiyue Energy unplanned downtime by 18%.",
        "baseline",
    ),
    SourceEpisode(
        "2026-03-05T11:00:00Z",
        "Haiyue Energy confirmed Lanzhou Technologies as the SmartPatrol developer, with Yu Chen leading delivery. "
        "Haiyue Energy plans to become the first commercial customer following pilot validation.",
        "baseline",
    ),
    SourceEpisode(
        "2026-03-15T16:00:00Z",
        "The Lanzhou Technologies board approved SmartPatrol transition from pilot to commercial release on April 1, 2026. "
        "Lan Zhou signed the resolution as CEO.",
        "baseline",
    ),
    SourceEpisode(
        "2026-04-01T08:00:00Z",
        "SmartPatrol officially launched commercially today, exiting pilot stage. "
        "Haiyue Energy became the first commercial customer, with Yu Chen continuing delivery management.",
        "baseline",
    ),
    SourceEpisode(
        "2026-04-18T13:00:00Z",
        "Lanzhou Technologies announced operational metrics from Starport headquarters: SmartPatrol covers 60 turbines. "
        "Haiyue Energy remains partner and customer under HY-2026-01.",
        "baseline",
    ),
    SourceEpisode(
        "2026-04-30T17:00:00Z",
        "As of April 30, 2026, Lan Zhou continues as CEO of Lanzhou Technologies, "
        "headquartered in Starport; Yu Chen serves as SmartPatrol project lead.",
        "baseline",
    ),
]
...
TEMPORAL_UPDATES = [
    SourceEpisode(
        ...
        "The Lanzhou Technologies board announced that Lan Zhou stepped down as CEO effective May 10, 2026. "
        "Yu Chen succeeded as CEO, while Lan Zhou transitioned to Chief Strategic Advisor.",
        "leadership_change",
    ),
    SourceEpisode(
        "2026-06-01T09:00:00Z",
        "Effective June 1, 2026, Lanzhou Technologies relocated its headquarters from Starport to Seacity. "
        "The Starport facility now operates as an R&D center.",
        "headquarters_change",
    ),
    SourceEpisode(
        "2026-06-15T12:00:00Z",
        "Lanzhou Technologies and Haiyue Energy terminated agreement HY-2026-01 early on June 15, 2026. "
        "Haiyue Energy is no longer a joint deployment partner but remains an active SmartPatrol customer.",
        "partnership_change",
    ),
    SourceEpisode(
...
def _episode_to_batch_item(graph_id: str, item: SourceEpisode, index: int) -> BatchAddItem:
    return BatchAddItem(
        ...
        source_description="Synthetic Minds deep Zep Cloud validation corpus",
        metadata={
            "source": "synthetic_minds_zep_deep_validation",
            "phase": item.phase,
            "sequence": index,
        },
    )
...
def _add_and_wait(client: Any, graph_id: str, item: SourceEpisode, timeout: int) -> str:
    episode = client.graph.add(
        ...
        data=item.data,
        created_at=item.created_at,
        source_description="Synthetic Minds temporal Zep Cloud validation update",
        metadata={"source": "synthetic_minds_zep_deep_validation", "phase": item.phase},
    )
...
def _activities() -> Iterable[AgentActivity]:
    return [
        AgentActivity(
            ...
            action_args={"content": "New headquarters opened in Seacity today. SmartPatrol commercial services operating normally."},
            round_num=1,
        ...
        AgentActivity(
            ...
            action_args={
                "original_author_name": "Yu Chen",
                "original_content": "New headquarters opened in Seacity today. SmartPatrol commercial services operating normally.",
                "quote_content": "As Chief Strategic Advisor, I support Yu Chen and the new executive team.",
            },
            round_num=1,
        ...
        AgentActivity(
            ...
            action_args={
                "post_author_name": "Yu Chen",
                "post_content": "New headquarters opened in Seacity today. SmartPatrol commercial services operating normally.",
            },
            round_num=1,
        ...
        AgentActivity(
            ...
            action_args={
                "post_author_name": "Yu Chen",
                "post_content": "New headquarters opened in Seacity today. SmartPatrol commercial services operating normally.",
                "content": "Although our joint deployment agreement terminated, we remain a loyal SmartPatrol customer.",
            },
            round_num=2,
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    graph_id = args.graph_id or f"synthetic_minds_zep_deep_{stamp}"
    ...
    try:
        builder.create_graph("Synthetic Minds Zep Cloud Deep Validation", graph_id=graph_id)
        created = True
        ...
        try:
            ...
            result["empty_attribute_ontology_probe"] = "accepted_after_synthetic_minds_normalization"
        except Exception as error:
        ...
        baseline_ceo = client.graph.search(
            graph_id=graph_id,
            query="As of the end of April 2026, who served as Chief Executive Officer of Lanzhou Technologies?",
            scope="edges",
        ...
        if updater_stats["items_sent"] != 5 or updater_stats["pending_episode_count"] != 0:
            raise AssertionError(f"unexpected Synthetic Minds updater stats: {updater_stats}")
        result["synthetic_minds_updater"] = updater_stats
        print("[zep-deep] Synthetic Minds updater processed 5 mock activities", flush=True)
        ...
        edge_search = client.graph.search(
            graph_id=graph_id,
            query="What is the current CEO, current headquarters, and current relationship between Lanzhou Technologies and Haiyue Energy?",
            scope="edges",
            reranker="cross_encoder",
            limit=20,
        )
        node_search = client.graph.search(
            graph_id=graph_id,
            query="Lanzhou Technologies leadership persons",
            scope="nodes",
            reranker="rrf",
            limit=10,
        ...
        typed_edge_search = client.graph.search(
            graph_id=graph_id,
            query="What changes occurred in corporate executive leadership roles?",
            scope="edges",
            reranker="rrf",
            limit=10,
        ...
        auto_search = client.graph.search(
            graph_id=graph_id,
            query="Summarize latest management, headquarters, product, and Haiyue Energy relations for Lanzhou Technologies.",
            scope="auto",
            max_characters=3000,
        ...
        selected_node = next(
            (
                ...
                if node.name in ("Lanzhou Technologies", "Lanzhou Tech") and "Company" in (node.labels or [])
            ),
            max(
                final_nodes,
                key=lambda node: sum(
        ...
        if len(entity_context.related_edges) != len(complete_node_edges):
            raise AssertionError(
                "Synthetic Minds entity context omitted incoming or outgoing node edges"
            )
...
def _compact_result(result: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "synthetic_minds_updater": result.get("synthetic_minds_updater"),
        "final": {
            key: final.get(key)
            for key in (
                "node_count",
