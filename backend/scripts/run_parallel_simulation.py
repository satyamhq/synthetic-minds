...
Runs Twitter and Reddit simulations concurrently using shared configuration.

Features:
- Dual-platform (Twitter + Reddit) parallel simulation
- Enters command mode after simulation rather than exiting immediately
...
Usage:
    ...
    python run_parallel_simulation.py --config simulation_config.json --no-wait  # Exit immediately upon completion
...
# ============================================================
# Resolve Windows encoding: set UTF-8 before imports
# Fixes issue where OASIS reads files without explicit encoding
# ============================================================
import sys
import os
...
if sys.platform == 'win32':
    ...
    # Affects open() calls without explicit encoding
    ...
    # Note: Applied at startup to monkey-patch built-in open
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None, 
        ...
        """
        Wrap open() to default to UTF-8 for text mode
        Fixes third-party libraries reading files without specifying encoding
        """
        # Only defaults to UTF-8 for text mode without explicit encoding
...
# Script located in backend/scripts/
...
class MaxTokensWarningFilter(logging.Filter):
    """Filter camel-ai max_tokens warning"""
...
# Add filter before camel execution
...
def disable_oasis_logging():
    ...
    OASIS logging is verbose; we use action_logger
    """
...
# Twitter available actions (excluding INTERVIEW which is manual)
TWITTER_ACTIONS = [
...
# Reddit available actions (excluding INTERVIEW which is manual)
REDDIT_ACTIONS = [
    ActionType.LIKE_POST,
...
class ParallelIPCHandler:
    ...
    def poll_command(self) -> Optional[Dict[str, Any]]:
        ...
        # Get command files sorted by mtime
        command_files = []
    ...
    def _get_env_and_graph(self, platform: str):
        """
        Get env and agent_graph for designated platform
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Designated platform (optional)
                ...
                - None/unspecified: Interview both platforms and aggregate
            
        Returns:
            True on success, False on failure
        """
        # If platform is specified, interview that platform only
        ...
        if success_count > 0:
            ...
            print(f"  Interview complete: agent_id={agent_id}, successful_platforms={success_count}/{len(platforms_to_interview)}")
            return True
        else:
...
# Filtered non-core action types
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    Displays real entity names in actions.jsonl instead of Agent_X
...
def fetch_new_actions_from_db(
    ...
    """
    Fetch new actions from database and enrich with full context
    
    Args:
        db_path: Database file path
        last_rowid: Last read maximum rowid
        agent_names: agent_id -> agent_name mapping
        
    Returns:
        (actions_list, new_last_rowid)
        - actions_list: Action list containing agent_id, agent_name, action_type, action_args
    ...
    try:
        ...
        # Use rowid to track processed records
        # Avoids created_at formatting differences across platforms
        cursor.execute("""
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Simplify action_args retaining key fields without truncation
            simplified_args = {}
            ...
            # Enrich context info (post content, username, etc.)
...
def _enrich_action_context(
    ...
    """
    Enrich action context info (post content, username, etc.)
    ...
    try:
        ...
        # Follow user: enrich followee username
        ...
        # Mute user: enrich muted username
        ...
        # Create comment: enrich commented post info
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    """
    Create OpenAI LLM model to drive agent decisions
    ...
    Args:
        ...
        use_boost: Keep signature compatible
    """
    openai_api_key = os.environ.get("OPENAI_API_KEY") or os.environ.get("LLM_API_KEY", "")
    openai_model = os.environ.get("OPENAI_MODEL") or os.environ.get("LLM_MODEL_NAME") or config.get("llm_model", "gpt-4o-mini")
    
    if openai_api_key:
        os.environ["OPENAI_API_KEY"] = openai_api_key
    
    if not os.environ.get("OPENAI_API_KEY"):
        raise ValueError("OPENAI_API_KEY is not configured. Please set OPENAI_API_KEY in your .env file.")
...
def get_active_agents_for_round(
    ...
    """Select active agents for the current round based on time and configuration"""
...
async def run_twitter_simulation(
    ...
    Args:
        ...
        max_rounds: Maximum simulation rounds (optional truncation)
    ...
    # Get agent real name mapping from config
    agent_names = get_agent_names_from_config(config)
    # Use default name if agent not in config
    ...
    result.env = oasis.make(
        ...
        semaphore=30,  # Limit maximum concurrent LLM requests to prevent overload
    )
    ...
    total_actions = 0
    last_rowid = 0  # Track last processed rowid in DB
    
    # Executing initial events
    ...
    for round_num in range(total_rounds):
        ...
        if not active_agents:
            # Record round end when no active agents (actions_count=0)
            if action_logger:
        ...
        # Fetch and log executed actions from database
    ...
    # Keep environment open for interviews
    ...
    log_info(f"Simulation loop complete! Elapsed: {elapsed:.1f}ss, total actions: {total_actions}")
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='Maximum simulation rounds (optional truncation)'
    )
    parser.add_argument(
        '--no-wait',
        action='store_true',
        default=False,
        help='Exit immediately after simulation without entering command mode'
    )
    ...
    if args.twitter_only:
        ...
        # Run in parallel with independent platform loggers
    ...
    # Check whether entering wait-for-command mode
    if wait_for_commands:
        log_manager.info("")
        log_manager.info("=" * 60)
        log_manager.info("Entering wait-for-command mode - environment running")
        ...
        try:
            while not _shutdown_event.is_set():
                ...
                if not should_continue:
                    break
                # Use wait_for instead of sleep to handle shutdown_event
                try:
...
def setup_signal_handlers(loop=None):
    """
    Set signal handlers for clean SIGTERM / SIGINT exit
    
    Persistent simulation: stay open for interview commands
    When receiving termination signal:
    1. Notify asyncio loop to stop waiting
    2. Give program opportunity to clean up resources
    3. Exit cleanly
    """
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            _cleanup_done = True
            # Set event to notify asyncio loop
            if _shutdown_event:
                _shutdown_event.set()
        
        # Allow asyncio loop to exit cleanly
